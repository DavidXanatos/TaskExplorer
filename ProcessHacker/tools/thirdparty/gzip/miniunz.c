    ...
    )
{
    ...
    while (TRUE)
    {
        ...
        *p = 0;
        ...
        {
            wprintf(L"couldn't create directory %s\n",buffer);
            free(buffer);
            return 0;
        }
    ...
    )
{
    ...
    for (i=0;i<gi.number_entry;i++)
    {
        ...
        wprintf(L" %3lu%%  %2.2lu-%2.2lu-%2.2lu  %2.2lu:%2.2lu  %8.8lx   %s\n",
                ratio,
    ...
    )
{
    ...
    else
    {
        ...
        if (((*popt_overwrite)==0) && (err==UNZ_OK))
        {
            ...
            ftestexist = FOPEN_FUNC(write_filename, L"rb");
            ...
            if (ftestexist!=NULL)
            {
                ...
                do
                {
                    ...
                    wprintf(L"The file %s exists. Overwrite ? [y]es, [n]o, [A]ll: ",write_filename);
                    ret = scanf("%1s",answer);
                    if (ret != 1)
                    {
                       exit(EXIT_FAILURE);
        ...
        {
            fout = FOPEN_FUNC(write_filename, TEXT("wb"));

            /* some zipfile don't contain directory alone before file */
            if ((fout == NULL) && ((*popt_extract_without_path) == 0) && (filename_withoutpath != (wchar_t*)filename_inzip))
            {
    ...
    )
{
    ...
    {
        wprintf(L"file %s not found in the zipfile\n",filename);
        return 2;
    }
