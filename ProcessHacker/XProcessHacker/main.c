...
NTSTATUS FindPsProtectionOffset(
    ...
    )
{
    ...
    if (KphDynOsVersionInfo.dwMajorVersion < 6 ||
        (KphDynOsVersionInfo.dwMajorVersion == 6 && KphDynOsVersionInfo.dwMinorVersion < 3))
    {
        dprintf("PPL Unsupported on this OS version. Be glad!\n");
    }
    else
    {
        // Find the offset of the PS_PROTECTION field for the running kernel
        NTSTATUS Status = FindPsProtectionOffset(&PsProtectionOffset);
        if (!NT_SUCCESS(Status) && Status != STATUS_NO_MORE_ENTRIES)
        {
            dprintf("Failed to find the PS_PROTECTION offset for Windows %u.%u.%u.\n",
                KphDynOsVersionInfo.dwMajorVersion, KphDynOsVersionInfo.dwMinorVersion, KphDynOsVersionInfo.dwBuildNumber);
        }

        // Only Windows 10.0.15063.0 and later are afflicted with kernel-enforced MS code signing requirements.
        if (NT_SUCCESS(Status) && KphDynOsVersionInfo.dwBuildNumber >= 15063)
        {
            ...
            if (!NT_SUCCESS(Status) && Status != STATUS_NO_MORE_ENTRIES)
            {
                dprintf("Failed to find the SignatureLevel and SectionSignatureLevel offsets for Windows %u.%u.%u.\n",
                    KphDynOsVersionInfo.dwMajorVersion, KphDynOsVersionInfo.dwMinorVersion, KphDynOsVersionInfo.dwBuildNumber);
    ...
    RtlInitUnicodeString(&deviceName, L"\\Device\\XProcessHacker3");
