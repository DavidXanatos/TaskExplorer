...
void UpdateDotNetStatTree(CWinProcess* pProcess, const QMap<int, QTreeWidgetItem*>& PerfCounters)
{
    ...
    if (ClrV4)
    {
        if (OpenDotNetPublicControlBlock_V4(!!IsImmersive, ProcessHandle, ProcessId, &BlockTableAddress))
        {
    ...
    {
        if (OpenDotNetPublicControlBlock_V2(ProcessId, &BlockTableAddress))
        {
            ControlBlockValid = TRUE;
        }
    }
