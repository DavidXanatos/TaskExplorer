...
{
    PET_GPU_COUNTER entry1 = (PET_GPU_COUNTER)Entry1;
    PET_GPU_COUNTER entry2 = (PET_GPU_COUNTER)Entry2;
...
{
    PET_GPU_COUNTER entry = (PET_GPU_COUNTER)Entry;
    ...
    )
{
    ...
    if (entry = (PET_GPU_COUNTER)PhFindEntryHashtable(EtGpuRunningTimeHashTable, &lookupEntry))
    {
        value = entry->Value;
    }
    ...
    )
{
    ...
    if (entry = (PET_GPU_COUNTER)PhFindEntryHashtable(EtGpuDedicatedHashTable, &lookupEntry))
    {
        value = entry->Value;
    }
    ...
    )
{
    ...
    if (entry = (PET_GPU_COUNTER)PhFindEntryHashtable(EtGpuSharedHashTable, &lookupEntry))
    {
        value = entry->Value;
    }
...
{  
    ...
    if (entry = (PET_GPU_COUNTER)PhFindEntryHashtable(EtGpuCommitHashTable, &lookupEntry))
    {
        value = entry->Value;
    }
    ...
    )
{
    ...
    if (pidPartSr.Length)
    {
        ...
        if (PhStringToInteger64(&pidPartSr, 10, (PLONG64)&processId))
        {
    ...
    )
{
    ...
    {
        buffer = (PPDH_FMT_COUNTERVALUE_ITEM)PhAllocate(bufferLength);
    ...
    )
{
    ...
    if (EtGpuRunningTimeHashTable)
    {
            ...
            ))
        {
            ...
            {
                PPDH_FMT_COUNTERVALUE_ITEM entry = (PPDH_FMT_COUNTERVALUE_ITEM)PTR_ADD_OFFSET(buffer, sizeof(PDH_FMT_COUNTERVALUE_ITEM) * i);
