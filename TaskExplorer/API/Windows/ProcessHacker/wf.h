...
#define _FW_VERSION(l, h) ((UINT_PTR)(ULONG)MAKELONG(l, h))
#define FW_VISTA_BINARY_VERSION _FW_VERSION(2,0) // 0x0200
...
typedef FW_POLICY_STORE_HANDLE *PFW_POLICY_STORE_HANDLE;

typedef enum _FW_POLICY_STORE_FLAGS
{
...
typedef    //[switch_type(FW_PROFILE_CONFIG)]
...
FWDeleteFirewallRule(
    _In_ FW_POLICY_STORE_HANDLE hPolicyStore, 
    _In_ const __nullterminated PWCHAR wszRuleID
    );
...
typedef ULONG (NTAPI *_FWEnumFirewallRules)(
    ...
    _In_ FW_ENUM_RULES_FLAGS wFlags,
    _Out_ PULONG pdwNumRules,
    _Out_ PFW_RULE *ppFwRules
    );
...
FWGetConfig(
    _In_ FW_POLICY_STORE_HANDLE hPolicyStore, 
    _In_ FW_PROFILE_CONFIG configId, 
    _In_ FW_PROFILE_TYPE Profile, 
    _In_ FW_CONFIG_FLAGS dwFlags, 
    __inout PCHAR pBuffer, 
    _In_ ULONG pdwBufSize
    );
...
FWSetConfig(
    ...
    _In_ FW_PROFILE_CONFIG configId, 
    _In_ FW_PROFILE_TYPE Profile, 
    _In_ FW_PROFILE_CONFIG_VALUE pConfig, 
    _In_ ULONG dwBufSize
    );
...
FWAddConnectionSecurityRule(
    _In_ FW_POLICY_STORE_HANDLE hPolicy, 
    _In_ PFW_CS_RULE2_0 pRule
    );
...
FWDeleteConnectionSecurityRule(
    _In_ FW_POLICY_STORE_HANDLE hPolicy, 
    _In_ __nullterminated PWCHAR pRuleId
    );
...
FWDeleteAuthenticationSet(
    _In_ FW_POLICY_STORE_HANDLE hPolicy, 
    _In_ FW_IPSEC_PHASE IpSecPhase, 
    _In_ const PWCHAR wszSetId
    );
...
FWDeleteCryptoSet(
    _In_ FW_POLICY_STORE_HANDLE hPolicy, 
    _In_ FW_IPSEC_PHASE IpSecPhase,
    _In_ const __nullterminated PWCHAR wszSetId
    );
...
NTAPI
FWExportPolicy(
    _In_ CONST __nullterminated PWCHAR wszMachineOrGPO,
    _In_ BOOLEAN fGPO, 
    _In_ CONST __nullterminated PWCHAR wszFilePath, 
...
FWRestoreGPODefaults(
    _In_ CONST __nullterminated PWCHAR wszGPOPath
    );
...
FwIsGroupPolicyEnforced(
    _In_ CONST __nullterminated PWCHAR wszMachine, 
    _In_ ULONG Profiles, 
