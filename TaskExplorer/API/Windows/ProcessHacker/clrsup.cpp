/*
 * Process Hacker -
 *   qt port CLR data access functions
 *
    ...
    )
{
    ...
    support = (PCLR_PROCESS_SUPPORT)PhAllocate(sizeof(CLR_PROCESS_SUPPORT));
    ...
    )
{
    ...
    appDomain = (IXCLRDataAppDomain *)AppDomain;
    ...
    )
{
    ...
    if (clrV4)
    {
        ...
        {
            mscordacwksDllBase = (HMODULE)LoadMscordacwks(TRUE);
    ...
    else
    {
        ...
        if (PhBeginInitOnce(&initOnce))
        {
            mscordacwksDllBase = (HMODULE)LoadMscordacwks(FALSE);
            PhEndInitOnce(&initOnce);
        }
    ...
    clrDataCreateInstance = (HRESULT (__stdcall *)(REFIID, ICLRDataTarget *, void **))PhGetProcedureAddress(dllBase, "CLRDataCreateInstance", 0);

    if (!clrDataCreateInstance)
        return E_FAIL;

    return clrDataCreateInstance(*(_GUID*)&IID_IXCLRDataProcess, Target, (void**)DataProcess);
}
    ...
    )
{
    ...
    dataTarget = (DnCLRDataTarget*)PhAllocate(sizeof(DnCLRDataTarget));
...
{
    if (
        IsEqualIID(Riid, *(_GUID*)&IID_IUnknown) ||
        IsEqualIID(Riid, *(_GUID*)&IID_ICLRDataTarget_I)
        )
    {
...
ULONG STDMETHODCALLTYPE DnCLRDataTarget_AddRef(
    _In_ ICLRDataTarget *_This
    )
{
    DnCLRDataTarget *This = (DnCLRDataTarget *)_This;
...
HRESULT STDMETHODCALLTYPE DnCLRDataTarget_GetPointerSize(
    _In_ ICLRDataTarget *This_,
    _Out_ ULONG32 *pointerSize
    )
{
    DnCLRDataTarget *This = (DnCLRDataTarget *)This_;
...
{
    PPHP_GET_IMAGE_BASE_CONTEXT context = (PPHP_GET_IMAGE_BASE_CONTEXT)Context;
