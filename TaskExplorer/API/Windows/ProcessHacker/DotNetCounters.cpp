    ...
    )
{
    if (Wow64)
    {
        LegacyPrivateIPCControlBlock_Wow64* IpcBlock = (LegacyPrivateIPCControlBlock_Wow64*)IpcBlockAddress;
    ...
    {
        LegacyPrivateIPCControlBlock* IpcBlock = (LegacyPrivateIPCControlBlock*)IpcBlockAddress;
    ...
    )
{
    if (Wow64)
    {
        LegacyPublicIPCControlBlock_Wow64* ipcLegacyBlockTable_Wow64 = (LegacyPublicIPCControlBlock_Wow64*)BlockTableAddress;
    ...
    {
        LegacyPublicIPCControlBlock* ipcLegacyBlockTable = (LegacyPublicIPCControlBlock*)BlockTableAddress;
    ...
    )
{
    if (Wow64)
    {
        IPCControlBlockTable_Wow64* ipcBlockTable_Wow64 = (IPCControlBlockTable_Wow64*)BlockTableAddress;
    ...
    {
        IPCControlBlockTable_Wow64* ipcBlockTable = (IPCControlBlockTable_Wow64*)BlockTableAddress;
    ...
    )
{
    ...
    if (Wow64)
    {
        ...
        appDomainEnumBlock = (AppDomainEnumerationIPCBlock_Wow64*)GetLegacyBlockTableEntry(
            Wow64,
            ipcControlBlockTable,
    ...
    else
    {
        ...
        appDomainEnumBlock = (AppDomainEnumerationIPCBlock*)GetLegacyBlockTableEntry(
            Wow64,
            ipcControlBlockTable,
            eLegacyPrivateIPC_AppDomain
            );
...
void QTreeWidget__AddListViewGroup(QTreeWidgetEx* pTree, QMap<int, QTreeWidgetItem*>& Items, int ID, const wchar_t* pLabel)
{
	...
	pItem->setText(0, QString::fromWCharArray(pLabel));
	pTree->addTopLevelItem(pItem);
	Items.insert(ID, pItem);
}

void QTreeWidget__AddListViewGroupItem(QMap<int, QTreeWidgetItem*>& Items, int ParentID, int ID, const wchar_t* pLabel, void*)
{
	QTreeWidgetItem* pItem = new QTreeWidgetItem();
	pItem->setText(0, QString::fromWCharArray(pLabel));
	Items[ParentID]->addChild(pItem);
	Items.insert(ID, pItem);
}

void InitDotNetStatTree(QTreeWidgetEx* pTree, QMap<int, QTreeWidgetItem*>& PerfCounters)
{
#define PhAddListViewGroup QTreeWidget__AddListViewGroup

#define ListViewHandle pTree, PerfCounters
...
#undef ListViewHandle

#define PhAddListViewGroupItem QTreeWidget__AddListViewGroupItem
#define ListViewHandle PerfCounters
...
void UpdateDotNetStatTree(CWinProcess* pProcess, const QMap<int, QTreeWidgetItem*>& PerfCounters)
{
...
#endif
	BOOLEAN IsImmersive = pProcess->IsImmersiveProcess();
    ...
    if (ClrV4)
    {
        if (OpenDotNetPublicControlBlock_V4(!!IsImmersive, ProcessHandle, ProcessId, &BlockTableHandle, &BlockTableAddress))
        {
    ...
    {
        if (OpenDotNetPublicControlBlock_V2(ProcessId, &BlockTableHandle, &BlockTableAddress))
        {
            ControlBlockValid = TRUE;
        }
    }
    ...
    if (ControlBlockValid)
    {
		...
		if (ClrV4)
		{
			perfStatBlock = GetPerfIpcBlock_V4(IsWow64, BlockTableAddress);
		}
		else
		{
			perfStatBlock = GetPerfIpcBlock_V2(IsWow64, BlockTableAddress);
		}
		...
		if (perfStatBlock)
		{
			...
			SContext TempContext;


			SContext* Context = &TempContext;
			if (IsWow64)
			{
			...
			SContext* context = &TempContext;
			for (QMap<int, QTreeWidgetItem*>::const_iterator I = PerfCounters.begin(); I != PerfCounters.end(); I++)
			{
				switch (I.key())
				{
					case DOTNET_INDEX_EXCEPTIONS_THROWNCOUNT:
					{
						...
						{
							I.value()->setText(1, QString::fromWCharArray(formatBuffer));
						}
					}
					break;
					...
					case DOTNET_INDEX_JIT_TIME:
					{
						...
						{
							I.value()->setText(1, QString::fromWCharArray(L"0.00"));
						}
					}
					break;
