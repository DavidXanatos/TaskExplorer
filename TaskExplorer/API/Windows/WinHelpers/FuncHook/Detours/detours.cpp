...
PBYTE detour_gen_jmp_immediate(PBYTE pbCode, PBYTE *ppPool, PBYTE pbJmpVal)
{
    ...
    *((PBYTE*&)pbLiteral) = pbJmpVal;
    ...
    write_opcode(pbCode, 0x58000011 | (((delta / 4) & 0x7FFFF) << 5));  // LDR X17,[PC+n]
...
{
    ULONG Opcode = fetch_opcode(pbCode);
    if ((Opcode & 0xfffffc1f) == 0xd61f0000 ||      // br <reg> // not 0xd65f0000
...
LONG WINAPI DetourUpdateThread(_In_ HANDLE hThread)
{
    ...
    DetourThread *t = (DetourThread*)malloc(sizeof(DetourThread));
    if (t == NULL) {
                           ...
                           _Out_opt_ PVOID *ppRealDetour)
{
    ...
    o = (DetourOperation*)malloc(sizeof(DetourOperation));
    if (o == NULL) {
        error = ERROR_NOT_ENOUGH_MEMORY;
      fail:
        s_nPendingError = error;
                         ...
                         _In_ PVOID pDetour)
{
    ...
    DetourOperation *o = (DetourOperation*)malloc(sizeof(DetourOperation));
    if (o == NULL) {
        error = ERROR_NOT_ENOUGH_MEMORY;
      fail:
        s_nPendingError = error;
