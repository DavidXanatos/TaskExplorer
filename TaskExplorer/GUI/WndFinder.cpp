...
#include <windows.h>
#include "../API/Windows/WinHelpers/FindTool.h"
...
UINT CALLBACK FindProc(HWND hwndTool, UINT uCode, HWND hwnd)
{
	...
	if(pid)
		QMetaObject::invokeMethod(theGUI, "OpenTaskInfoWnd", Qt::QueuedConnection, Q_ARG(quint64, pid));
	...
	//	else
	//	{
	//		std::wstring result = CTaskExplorer::tr("The selected window is not running as part of any sandboxed program.").toStdWString();
	//	
	//		SetWindowTextW(GetDlgItem(hwndTool, ID_FINDER_RESULT), result.c_str());
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		case WM_CREATE:
		{
			...
			std::wstring info = CTaskExplorer::tr("Drag the Finder Tool over a window to select it, then release the mouse to open process properties.").toStdWString();
...
void CTaskExplorer::OnWndFinder()
{
	m_pMenuFindWnd->setEnabled(false);
	...
	connect(finishedNotifier, &QWinEventNotifier::activated, this, [finishedNotifier, this, hThread]() {
		...
		m_pMenuFindWnd->setEnabled(true);
